...
const ROLES = ['player', 'manager', 'selector', 'admin'];
...
export default function PublicMembers() {
  ...
  useEffect(() => {
    api.get('/public/members').then(r => { setMembers(r.data); setLoading(false); });
  }, []);

  const filtered = members.filter(m => {
    const q = search.toLowerCase();
    const matchSearch = m.name.toLowerCase().includes(q) || (m.bio || '').toLowerCase().includes(q);
    const memberRoles: string[] = m.roles ?? [m.role];
    const matchRole = roleFilter === 'all' || memberRoles.includes(roleFilter);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        {/* Filters — role filter only shown to logged-in users */}
        <div className="flex flex-col sm:flex-row gap-3 mb-8">
          <input className="input-field max-w-xs" placeholder="Search members…"
              ...
              return (
                <div key={m.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-shadow flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {/* Roles shown only when logged in */}
                    {user && (
                      <div className="flex flex-wrap gap-1 mt-0.5">
                        {memberRoles.map(r => (
                          <span key={r} className={`text-xs ${roleBadge[r] ?? 'badge-player'}`}>{r}</span>
                        ))}
                      </div>
                    )}
